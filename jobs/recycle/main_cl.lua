...
Citizen.CreateThread(function ()
    ...
    while true do
        ...
        if #(pos - vector3(582.6, 267.87, 5.73)) < 15 and not IsPedInAnyVehicle(PlayerPedId(), false) and carryPackage == nil then
            DrawMarker(2, 1049.15, -3100.63, -39.20, 0.9, 0, 0, 0, 0, 0, 0.2001, 0.2001, 0.2001, 255, 255, 255, 255, 0, 0, 0, 0)
            if #(pos - vector3(582.6, 267.87, 5.73)) < 1.3 then
                if onDuty then
                    DrawText3D(582.6, 267.87, 5.73, "Appuyez sur [~g~E~w~] pour arrêter de travailler")
                else
                    DrawText3D(582.6, 267.87, 5.73, "Appuyez sur [~g~E~w~] pour démarrer le travail")
                end
                if IsControlJustReleased(0, 38) then
                    onDuty = not onDuty
                    if onDuty then
                        QBCore.Functions.Notify("Vous êtes désormais au travail.", "success")
                    else
                        QBCore.Functions.Notify("Vous avez pris votre journée.", "error")
                    end
                end
            end
        end
...
RegisterNetEvent('ilfr-core:recycle:onduty')
AddEventHandler('ilfr-core:recycle:onduty', function()
onDuty = not onDuty
end)
            ...
            if packagePos ~= nil then
                ...
                if carryPackage == nil then
                    if #(pos - vector3(packagePos.x, packagePos.y, packagePos.z)) < 2.3 then
                        DrawText3D(packagePos.x,packagePos.y,packagePos.z+ 1, "Appuyez sur [~g~E~w~] pour prendre un carton")
                        if IsControlJustReleased(0, 38) then
                            QBCore.Functions.Progressbar("pickup_reycle_package", "Vous ramassez un carton", math.random(4000, 6000), false, true, {
                        ...
                        DrawText3D(packagePos.x, packagePos.y, packagePos.z + 1, "Matériel à recycler")
                    end
                else
                    if #(pos - vector3(Config['delivery'].dropLocation.x, Config['delivery'].dropLocation.y, Config['delivery'].dropLocation.z)) < 2.0 then
                        DrawText3D(Config['delivery'].dropLocation.x, Config['delivery'].dropLocation.y, Config['delivery'].dropLocation.z, "Appuyez sur [~g~E~w~] pour ouvrir et stocker le matériel")
                        if IsControlJustReleased(0, 38) then
                            DropPackage()
                            ScrapAnim()
                            QBCore.Functions.Progressbar("deliver_reycle_package", "Déballage et stockage du matériel", 5000, false, true, {
                                disableMovement = true,
                                disableCarMovement = true,
                                disableMouse = false,
                                disableCombat = true,
                        ...
                        DrawText3D(Config['delivery'].dropLocation.x, Config['delivery'].dropLocation.y, Config['delivery'].dropLocation.z, "Recyclage du matériel")
                    end
                end
            else
                GetRandomPackage()
