...
RegisterNetEvent('qb-djbooth:server:playMusic', function(song, zoneName)
    ...
    local boothCoords = Config.LocationsAudio[zoneName].coords
    local dist = #(coords - boothCoords)
    if dist > 3 then return end
    xSound:PlayUrlPos(-1, zoneName, song, Config.DefaultVolume, coords)
    xSound:Distance(-1, zoneName, Config.LocationsAudio[zoneName].radius)
    Config.LocationsAudio[zoneName].playing = true
...
RegisterNetEvent('qb-djbooth:server:stopMusic', function(data)
    ...
    local ped = GetPlayerPed(src)
    local coords = GetEntityCoords(ped)
    local boothCoords = Config.LocationsAudio[data.zoneName].coords
    local dist = #(coords - boothCoords)
    if dist > 3 then return end
    if Config.LocationsAudio[data.zoneName].playing then
        Config.LocationsAudio[data.zoneName].playing = false
...
RegisterNetEvent('qb-djbooth:server:resumeMusic', function(data)
    ...
    local dist = #(coords - boothCoords)
    if dist > 3 then return end
    if not Config.LocationsAudio[data.zoneName].playing then
        Config.LocationsAudio[data.zoneName].playing = true
...
RegisterNetEvent('qb-djbooth:server:changeVolume', function(volume, zoneName)
    ...
    if Config.LocationsAudio[zoneName].playing then
